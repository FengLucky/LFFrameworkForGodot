...
namespace LF.Runtime
{
    public static class Archive
    {
        ...
        /// <typeparam name="TV">简易存档类型</typeparam>
        /// <returns></returns>
        private static TV[] LoadAllSimpleArchive<T, TV>() where T : ArchiveData where TV : ArchiveSimpleData<T,TV>, new()
        {
            ...
            var allFilePath = Directory.GetFiles(ArchivePath, "*.json");
            ...
            {
                var archive = LoadArchive<T>(path);
                if (archive != null)
                {
                    var simpleArchive = new TV();
                    simpleArchive.ConvertFrom(archive);
                    result.Add(simpleArchive);
                }
            }
